...
class UtilTestSpec extends FunSpec with Matchers {
  describe("Test getSDSDatatableSchema") {
    it("should return expected list") {
      ...
      // TODO: find a way to compare two vectors
      actual.foreach { datatableSchema =>
        expected should contain((datatableSchema.getColumnName,
          datatableSchema.getType, datatableSchema.isAnonymize))
      }
    }
  }

  describe("Test setSDSDatatableSchema") {
    it("should set list") {
      val conf = new Configuration()
      val expected = Vector(
        new DatatableSchema("column_1", "string", true),
  ...
  describe("Test getNewDataSchemaFromDatatableSchema") {
    it("should ignore fields which are not exist in datatable schema sequence") {
      ...
      println("actual")
      actual.fields.foreach(f => printf("%s \n", f.name))
      println("actual")
      println("expected")
      expected.fields.foreach(f => printf("%s \n", f.name))
      println("expected")
    }
  }
}
