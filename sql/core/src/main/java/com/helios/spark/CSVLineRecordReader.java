...
public class CSVLineRecordReader extends RecordReader<LongWritable, Text> {
  ...
  public boolean nextKeyValue() throws IOException {
    ...
    // Note: [TestForSDSCSVSuite] Comment modifyText to run test
    this.value = removeAndAnonymizeColumn(this.value);
    return true;
  }
  ...
  private Text removeAndAnonymizeColumn(Text text) {
    if (text.getLength() == 0) {
      return text;
    }
